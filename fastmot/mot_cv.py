...
from .models import yolo_cv, osnet_cv
...
class MOT:
    ...
    def __init__(self, size, capture_dt, config, draw=False, verbose=False):
        ...
        self.detector = yolo_cv.Yolo_CV(self.size, config['yolo_cv'])

        LOGGER.info('Loading feature extractor model...')
        self.extractor = osnet_cv.OSNet_CV(config['osnet_cv'])
    ...
    def step(self, frame):
        ...
        if self.frame_count == 0:
            ...
            if self.frame_count % self.detector_frame_skip == 0:
                ...
                detections = self.detector.detect(self.frame_count, frame)
